...
/** Devuelve true si el JWT corresponde a un usuario ADMIN */
...
/* ─────────────────────────────────────────────────────────
   ...
   Admin autenticado puede ver también los inactivos con ?all=1
───────────────────────────────────────────────────────── */
app.get("/api/v1/store/products", async (req, res) => {
  try {
    // Intentar obtener el usuario del token (opcional en este endpoint)
    let showAll = false;
    ...
    if (authHeader?.startsWith("Bearer ")) {
      try {
        ...
        if (payload.role === "ADMIN" && req.query.all === "1") showAll = true;
      } catch (_) { /* token inválido o ausente — ok, continuar como público */ }
    }

    const where = showAll ? "1=1" : "is_active = 1";
...
/* ─────────────────────────────────────────────────────────
   POST /api/v1/store/products  (JWT + ADMIN)
   Crea un nuevo producto.
───────────────────────────────────────────────────────── */
app.post("/api/v1/store/products", authRequired, async (req, res) => {
  if (!isAdminUser(req))
    return res.status(403).json({ error: "FORBIDDEN" });
  ...
  try {
    ...
    const [ins] = await pool.execute(
      `INSERT INTO store_products (name, description, price, stock, category, image_url, is_active)
       VALUES (:name, :description, :price, :stock, :category, :image_url, :is_active)`,
      {
        name:        name.trim(),
        description: description?.trim() ?? "",
        price:       parseFloat(price),
        stock:       parseInt(stock, 10),
        category:    category?.trim() ?? "Accesorios",
        image_url:   image_url?.trim() ?? null,
        is_active:   is_active !== undefined ? (is_active ? 1 : 0) : 1,
      }
    );

    await auditLog("STORE_PRODUCT_CREATE", req.user.id,
      `Producto creado en tienda: ${name}`, { productId: ins.insertId });

    return res.status(201).json({ ok: true, id: ins.insertId });
  } catch (e) {
    console.error("[Store/Products/Create]", e);
    return res.status(500).json({ error: "SERVER_ERROR" });
  }
});

/* ─────────────────────────────────────────────────────────
   PATCH /api/v1/store/products/:id  (JWT + ADMIN)
   Actualiza un producto existente.
───────────────────────────────────────────────────────── */
app.patch("/api/v1/store/products/:id", authRequired, async (req, res) => {
  if (!isAdminUser(req))
    return res.status(403).json({ error: "FORBIDDEN" });
  ...
  try {
    ...
    const [check] = await pool.execute(
      `SELECT id FROM store_products WHERE id = :id LIMIT 1`,
      { id }
    );
    if (!check.length)
    ...
    await pool.execute(
       ...
       WHERE id = :id`,
      {
        ...
        image_url:   image_url   != null ? image_url.trim()   : null,
        is_active:   is_active   != null ? (is_active ? 1 : 0): null,
        id,
      }
    );

    await auditLog("STORE_PRODUCT_UPDATE", req.user.id,
      `Producto actualizado en tienda #${id}`, { productId: id });

    return res.json({ ok: true });
  } catch (e) {
    console.error("[Store/Products/Update]", e);
    return res.status(500).json({ error: "SERVER_ERROR" });
  }
});

/* ─────────────────────────────────────────────────────────
   DELETE /api/v1/store/products/:id  (JWT + ADMIN)
   Elimina un producto.
───────────────────────────────────────────────────────── */
app.delete("/api/v1/store/products/:id", authRequired, async (req, res) => {
  if (!isAdminUser(req))
    return res.status(403).json({ error: "FORBIDDEN" });
  ...
  try {
    ...
    await pool.execute(`DELETE FROM store_products WHERE id = :id`, { id });

    await auditLog("STORE_PRODUCT_DELETE", req.user.id,
      `Producto eliminado de tienda #${id}`, { productId: id });
