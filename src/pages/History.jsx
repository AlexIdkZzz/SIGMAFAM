...
const STATUS_LABEL = {
  RECEIVED: { label: "Recibida",   color: "bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400" },
  ACTIVE:   { label: "Activa",     color: "bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400" },
  ATTENDED: { label: "Atendida",   color: "bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400" },
  CLOSED:   { label: "Cerrada",    color: "bg-slate-100 text-slate-500 dark:bg-slate-800 dark:text-slate-400" },
};
...
export default function History() {
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto transition-colors">

      {/* Header */}
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-extrabold text-slate-900 dark:text-white">Historial de Alertas</h1>
        ...
        <button
          ...
          disabled={loading}
          className="flex items-center gap-2 px-3 py-2 rounded-xl border border-slate-200 dark:border-slate-700 text-sm text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 disabled:opacity-50 transition"
        >
      ...
      {/* Error */}
      {error && (
        <div className="mb-4 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800/50 text-red-700 dark:text-red-400 text-sm rounded-xl px-4 py-3">
          {error}
        </div>
      )}

      {/* Tabla */}
      <div className="bg-white dark:bg-[#0a0f1e] border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden shadow-sm transition-colors">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50">
                  <th className="text-left px-4 py-3 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide">ID</th>
                  ...
                  <th className="text-left px-4 py-3 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide">Origen</th>
                  ...
                  <th className="text-left px-4 py-3 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide">Creada</th>
                  <th className="text-left px-4 py-3 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide">Cerrada</th>
                  <th className="text-left px-4 py-3 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide">Ubicación</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800/50">
                {alerts.map((a) => {
                  const st  = STATUS_LABEL[a.status]  ?? { label: a.status,  color: "bg-slate-100 text-slate-500 dark:bg-slate-800 dark:text-slate-400" };
                  ...
                  return (
                    <tr key={a.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                      ...
                      <td className="px-4 py-3 font-medium text-slate-800 dark:text-slate-200">{a.user}</td>
                      ...
                      <td className="px-4 py-3 text-slate-500 dark:text-slate-400">{formatDate(a.closedAt)}</td>
        ...
        <div className="flex items-center justify-between mt-4">
          ...
          <div className="flex gap-2">
            <button
              ...
              className="px-3 py-1.5 rounded-xl border border-slate-200 dark:border-slate-700 text-sm text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 disabled:opacity-40 transition"
            >
              ← Anterior
            </button>
            <button
