...
function RolePill({ role }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest border transition-all ${
        isJefe
...
function NoGroup({ token, onRefresh }) {
  ...
  const inputClasses =
    "w-full px-5 py-4 rounded-2xl border transition-all outline-none " +
    "bg-slate-50 dark:bg-[#0a0f1e] border-slate-200 dark:border-slate-800 " +
    "text-slate-900 dark:text-white text-sm font-black uppercase " +
          ...
          <div className="space-y-2">
            {[
              ...
              <button
                ...
                className="w-full flex items-center gap-5 p-5 rounded-2xl border border-transparent hover:border-blue-500/30 bg-slate-50 dark:bg-slate-900/40 hover:dark:bg-blue-600/5 transition-all group"
              >
                <div className="w-12 h-12 rounded-xl bg-white dark:bg-[#050a18] flex items-center justify-center border border-slate-200 dark:border-slate-800 group-hover:bg-blue-600 group-hover:border-blue-600 group-hover:text-white transition-all shadow-sm text-slate-600 dark:text-slate-400">
                  <btn.icon size={22} />
                </div>
                <div className="text-left">
                  <p className="font-black text-slate-900 dark:text-white text-[11px] uppercase tracking-widest">
                    {btn.title}
                  </p>
                  <p className="text-[9px] font-bold text-slate-500 dark:text-slate-500 uppercase mt-0.5">
                    {btn.label}
                  </p>
                </div>
              </button>
          ...
          <div className="p-4 space-y-4">
            ...
            <div className="flex gap-2">
              <Button
                ...
                disabled={loading}
                className="flex-1 font-black text-[10px] tracking-widest uppercase py-4 bg-slate-900 dark:bg-blue-600 text-white shadow-lg shadow-blue-500/10"
              >
                {loading ? <Loader2 className="animate-spin mx-auto" size={16} /> : "Confirmar Acceso"}
              </Button>
              <button
                className="px-6 py-4 border border-slate-200 dark:border-slate-800 rounded-2xl font-black text-[10px] tracking-widest uppercase text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-white/5 transition-all"
...
export default function Family() {
  ...
  async function removeMember(memberId) {
    ...
    try {
      const res = await fetch(`${API_BASE}/family/member/${memberId}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` },
      });
    ...
    <PageShell
      ...
      subtitle={
        <span className="text-slate-500 dark:text-slate-500 font-black uppercase tracking-[0.2em] text-[10px] italic">
          MIEMBROS ACTIVOS: {group.members.length} / 6
        </span>
      }
      right={
    ...
    >
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 animate-in fade-in slide-in-from-bottom-2 duration-500">
              ...
              <Button
                ...
                className={`w-full py-5 font-black uppercase text-[10px] tracking-widest rounded-2xl transition-all text-white ${
                  copied
                    ...
                    : "bg-slate-900 dark:bg-blue-600 shadow-lg shadow-blue-500/20"
                }`}
              >
        ...
        <div className="lg:col-span-2">
          <Card className="overflow-hidden border-slate-200 dark:border-slate-800 bg-white dark:bg-[#0d1426] shadow-2xl p-0">
            <div className="overflow-x-auto">
              <table className="w-full text-left">
                <thead>
                  <tr className="bg-slate-50 dark:bg-[#050a18] border-b border-slate-100 dark:border-slate-800">
                    <th className="p-6 text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest italic">
                      Identidad Digital
                    </th>
                    <th className="p-6 text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest hidden sm:table-cell">
                      Rango / Alta
                    </th>
                    <th className="p-6" />
                  </tr>
                    ...
                    >
                      <td className="p-6">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <p className="font-black text-slate-900 dark:text-white uppercase text-[12px] tracking-tight">
                              {m.fullName}
                            </p>
