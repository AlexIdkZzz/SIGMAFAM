...
function MetricCard({ label, value, sub, icon: Icon, colorClass, borderSide }) {
  return (
    <div className="relative overflow-hidden bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-[2rem] p-6 shadow-xl shadow-slate-200/50 dark:shadow-none group transition-all duration-300 hover:-translate-y-1">
      {/* CORRECCIÓN: Agregadas llaves y backticks */}
...
function RiskMapOverlay({ hotspots, onClose }) {
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 9999, backgroundColor: "#020617", display: "flex", flexDirection: "column" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 16, padding: "14px 24px", backgroundColor: "#0f172a", borderBottom: "1px solid #1e293b", flexShrink: 0 }}>
        <button onClick={onClose} style={{ display: "flex", alignItems: "center", gap: 10, background: "none", border: "none", cursor: "pointer" }}>
          <div style={{ width: 38, height: 38, borderRadius: 12, backgroundColor: "#1e293b", display: "flex", alignItems: "center", justifyContent: "center" }}>
            <ArrowLeft size={18} color="#94a3b8" />
          </div>
          <span style={{ fontSize: 13, fontWeight: 700, color: "#94a3b8" }}>Regresar</span>
        </button>
        <div style={{ width: 1, height: 22, backgroundColor: "#334155" }} />
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <Shield size={15} color="#f87171" />
          <span style={{ fontSize: 11, fontWeight: 900, color: "#f1f5f9", letterSpacing: "0.15em", textTransform: "uppercase" }}>Zonas de Riesgo</span>
        </div>
      </div>
      <div style={{ flex: 1, position: "relative" }}>
        <MapContainer center={[20.6400, -103.3300]} zoom={11} style={{ height: "100%", width: "100%" }} zoomControl>
          <TileLayer url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" />
          <GeoJSON data={COLONIAS_GEOJSON} style={geoJsonStyle} />
          {(hotspots ?? []).map((h, i) => (
            <Circle key={i} center={[Number(h.lat), Number(h.lng)]} radius={Math.max(50, Number(h.intensity) * 120)} pathOptions={{ color: "#ef4444", fillColor: "#ef4444", fillOpacity: 0.35, weight: 0 }}>
...
export default function Stats() {
  ...
  useEffect(() => {
    if (!token) return;
    /* CORRECCIÓN: Agregados backticks en fetch */
  ...
  return (
    <>
      {showRiskMap && <RiskMapOverlay hotspots={data.hotspots} onClose={() => setShowRiskMap(false)} />}
      <PageShell title="Dashboard Operativo" subtitle="Análisis predictivo e histórico de seguridad familiar.">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <MetricCard label="Total Histórico" value={data.total} sub="+12% vs mes anterior" icon={BarChart3} colorClass="bg-indigo-50 text-indigo-600 dark:bg-indigo-500/10 dark:text-indigo-400" borderSide="bg-indigo-500" />
          <MetricCard label="Atención Requerida" value={(statusMap.ACTIVE ?? 0) + (statusMap.RECEIVED ?? 0)} sub="Prioridad inmediata" icon={Activity} colorClass="bg-red-50 text-red-600 dark:bg-red-500/10 dark:text-red-400" borderSide="bg-red-500" />
          <MetricCard label="Casos Resueltos" value={statusMap.CLOSED ?? 0} sub="Tasa de éxito 98%" icon={CheckCircle2} colorClass="bg-emerald-50 text-emerald-600 dark:bg-emerald-500/10 dark:text-emerald-400" borderSide="bg-emerald-500" />
          {/* CORRECCIÓN: Agregadas llaves y backticks */}
          <MetricCard label="Latencia Media" value={`${data.avgResponseMinutes ?? 0}m`} sub="Tiempo de respuesta" icon={Clock} colorClass="bg-amber-50 text-amber-600 dark:bg-amber-500/10 dark:text-amber-400" borderSide="bg-amber-500" />
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
              ...
              <div className="h-[350px] mt-6">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={barData}>
                    ...
                    <YAxis hide />
                    <Tooltip cursor={{ fill: isDark ? "rgba(255,255,255,0.05)" : "rgba(148,163,184,0.1)" }} contentStyle={{ borderRadius: 20, border: "none", backgroundColor: isDark ? "#1e293b" : "#0f172a", color: "#fff", fontWeight: 800 }} itemStyle={{ color: "#fff" }} />
                    <Bar dataKey="Alertas" radius={[10, 10, 10, 10]} barSize={32}>
                      {barData.map((_, index) => (
                        /* CORRECCIÓN: Agregados backticks en key */
          ...
          <Card title="Estado del Sistema" icon={Activity} className="dark:bg-slate-900 dark:border-slate-800 transition-colors">
            <div className="space-y-6 mt-6">
              {(data.byStatus ?? []).map((s) => {
                const theme = STATUS_THEME[s.status] || { color: "#64748b", text: "text-slate-600 dark:text-slate-400" };
                ...
                return (
                  <div key={s.status}>
                    ...
                    <div className="h-3 bg-slate-50 dark:bg-slate-800 rounded-full overflow-hidden border border-slate-100 dark:border-slate-700 p-0.5">
                      {/* CORRECCIÓN: Agregado style con backticks */}
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-[2rem] p-6 shadow-xl shadow-slate-200/50 dark:shadow-none">
          <div className="flex items-center gap-3 mb-6">
            <div className="p-2 rounded-xl bg-slate-100 dark:bg-slate-800">
              <MapIcon size={18} className="text-slate-600 dark:text-slate-300" />
            </div>
            <h3 className="text-sm font-black uppercase tracking-widest text-slate-900 dark:text-white">Zonificación de Riesgo</h3>
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
            <div className="lg:col-span-3">
              {/* CORRECCIÓN: Agregados backticks en el border de style */}
              <div style={{ height: 420, borderRadius: 28, overflow: "hidden", border: `8px solid ${isDark ? "#1e293b" : "#f8fafc"}` }}>
                <MapContainer center={[20.6400, -103.3300]} zoom={11} style={{ height: "100%", width: "100%" }}>
                  <TileLayer url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" />
                  <GeoJSON data={COLONIAS_GEOJSON} style={geoJsonStyle} />
            ...
            <div className="flex flex-col justify-between">
              <div className="p-6 bg-slate-900 dark:bg-slate-800/80 rounded-[2rem] text-white border dark:border-slate-700">
                ...
                <p className="text-sm font-medium leading-relaxed opacity-80">Se han detectado <span className="text-red-400 font-black">{data.hotspots?.length} áreas</span> de reincidencia alta en el perímetro.</p>
                ...
                <button onClick={() => setShowRiskMap(true)} className="mt-6 w-full flex items-center justify-center gap-2 py-3 px-4 rounded-2xl bg-red-500 hover:bg-red-600 active:scale-95 transition-all text-white text-[11px] font-black uppercase tracking-widest">
                  <Shield size={14} /> Ver zonas de riesgo
                </button>
              </div>
            </div>
          </div>
