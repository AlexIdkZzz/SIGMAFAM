...
export default function Alerts() {
  ...
  return (
    ...
    >
      {/* Error banner con dark mode */}
      {error && (
        <div className="mb-3 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800/50 text-red-700 dark:text-red-400 text-sm rounded-xl px-4 py-2 transition-colors">
          {error}
        </div>
      )}
                ...
                alerts.map((a) => {
                  ...
                  return (
                    <tr
                      key={a.id}
                      className={[
                        "border-b border-slate-100 dark:border-slate-800/50 cursor-pointer transition-colors",
                        isSel 
                    ...
                    >
                      <td className="py-3 pr-3 font-semibold text-slate-900 dark:text-white">#{a.id}</td>
                      ...
                      <td className="py-3 pr-3">
                        <div className="flex justify-end gap-2">
                          <button
                            className="px-3 py-2 rounded-xl border border-slate-200 dark:border-slate-700 hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300 text-xs font-semibold transition-colors"
                            onClick={(e) => { e.stopPropagation(); selectAlert(a.id); setDrawerOpen(true); }}
                          >
                            Ver
                          </button>
                          <button
                            className="px-3 py-2 rounded-xl bg-blue-600 hover:bg-blue-700 dark:bg-blue-600 dark:hover:bg-blue-500 text-white text-xs font-semibold transition-colors"
                            onClick={(e) => { e.stopPropagation(); selectAlert(a.id); nav("/app/map"); }}
                          >
                            Mapa
                          </button>
                          <button
                            className="px-3 py-2 rounded-xl bg-red-600 hover:bg-red-700 dark:bg-red-600 dark:hover:bg-red-500 text-white text-xs font-semibold transition-colors"
      ...
      >
        {!selected ? (
          <div className="text-sm text-slate-600 dark:text-slate-400">Selecciona una alerta.</div>
        ) : (
              ...
              {selected.lastLocation ? (
                <>
                  <div className="text-sm text-slate-700 dark:text-slate-300 mb-2">
                    <div><b className="text-slate-900 dark:text-white">Lat:</b> {selected.lastLocation.lat}</div>
                    <div><b className="text-slate-900 dark:text-white">Lng:</b> {selected.lastLocation.lng}</div>
                    <div><b className="text-slate-900 dark:text-white">Hora:</b> {fmtTime(selected.lastLocation.at)}</div>
                  </div>
                  <MiniMap lat={selected.lastLocation.lat} lng={selected.lastLocation.lng} />
                </>
              ) : (
                <div className="text-sm text-slate-400 dark:text-slate-500">Sin ubicación registrada.</div>
              )}
            </Card>

            <Card title="Dispositivo">
              <div className="text-sm text-slate-700 dark:text-slate-300">
                <div><b className="text-slate-900 dark:text-white">Batería:</b> {typeof selected.battery === "number" ? `${selected.battery}%` : "N/A"}</div>
              </div>
            </Card>
