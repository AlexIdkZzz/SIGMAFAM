...
function StatCard({ label, value, sub, danger }) {
  return (
    <div className="bg-slate-100 dark:bg-slate-800 rounded-lg px-4 py-3.5">
      <p className="text-xs text-slate-500 dark:text-slate-400 mb-1">{label}</p>
      <p className={`text-2xl font-medium ${danger ? "text-red-500 dark:text-red-400" : "text-slate-900 dark:text-slate-100"}`}>{value ?? "—"}</p>
...
export default function AdminOverview() {
  ...
  if (loading) return <div className="text-sm text-slate-400 dark:text-slate-500 py-10 text-center">Cargando...</div>;
  if (error)   return <div className="text-sm text-red-500 dark:text-red-400 py-10 text-center">{error}</div>;
            ...
            <thead>
              <tr>
                {["Usuario", "Grupo", "Origen", "Estado", "Fecha"].map((h) => (
                  <th key={h} className="text-left text-[11px] text-slate-400 dark:text-slate-500 font-medium px-5 py-2.5 bg-slate-50 dark:bg-slate-800/50 border-b border-slate-100 dark:border-slate-800">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
