    ...
    <>
          ...
          <table className="w-full min-w-[700px] text-[13px]">
            ...
            <tbody>
              {loading ? <LoadingRows cols={7} /> :
               ...
               groups.length === 0 ? (
                <tr><td colSpan={7} className="px-5 py-10 text-center text-slate-400 dark:text-slate-500 text-sm">Sin grupos registrados</td></tr>
              ) : groups.map((g) => (
                <tr key={g.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 border-b border-slate-50 dark:border-slate-800 last:border-0">
                  <td className="px-5 py-3">
                    <div className="font-medium text-slate-900 dark:text-slate-100">{g.name}</div>
                    <div className="text-[11px] text-slate-400 dark:text-slate-500 font-mono">{g.invite_code}</div>
                  </td>
                  <td className="px-5 py-3 text-slate-600 dark:text-slate-300">{g.owner_name}</td>
                  <td className="px-5 py-3 font-medium text-slate-900 dark:text-slate-100">{g.member_count}</td>
                  <td className="px-5 py-3 text-slate-700 dark:text-slate-300">{g.device_count}</td>
                  <td className="px-5 py-3 text-slate-700 dark:text-slate-300">{g.alert_count}</td>
                  <td className="px-5 py-3 text-xs text-slate-400 dark:text-slate-500">{formatDate(g.created_at)}</td>
                  <td className="px-5 py-3">
      ...
      {modal?.type === "dissolve" && (
        <Modal title="Disolver grupo familiar" onClose={() => setModal(null)}
          ...
          <p className="text-sm text-slate-600 dark:text-slate-300 mb-2">¿Seguro que quieres disolver este grupo?</p>
          <p className="text-sm font-medium text-slate-900 dark:text-slate-100">{modal.group.name}</p>
          <p className="text-xs text-slate-400 dark:text-slate-500 mt-2">Todos los miembros quedarán sin grupo. Esta acción no se puede deshacer.</p>
        </Modal>
      )}
    </>
  );
    ...
    <Modal title={group.name} onClose={onClose} footer={<BtnSecondary onClick={onClose}>Cerrar</BtnSecondary>}>
      <div className="space-y-1">
        {!group.members?.length ? (
          <p className="text-sm text-slate-400 dark:text-slate-500">Sin miembros registrados.</p>
        ) : group.members.map((m) => (
          <div key={m.id} className="flex items-center justify-between py-2 border-b border-slate-100 dark:border-slate-800 last:border-0">
            <span className="flex items-center gap-2.5">
              ...
              <span>
                <div className="text-sm font-medium text-slate-900 dark:text-slate-100">{m.full_name}</div>
